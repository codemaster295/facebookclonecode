import { faCross, faExclamation, faExclamationCircle, faQuestionCircle, faTimes } from '@fortawesome/free-solid-svg-icons'
...
const SignUpPopUp = ({ closepopup }) => {
    ...
    const handleSignUpSubmit = async (e) => {
        ...
        if (age) {
            ...
            setError("you must be 18 years old to register the account")
            toast.error(error)
        }
    ...
    return (
        <Grow in={checked}>
            <div className="w-full z-50 SignUpPopUp fixed h-screen top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-bg-theme  flex justify-center items-center">
                <ToastContainer icon={<FontAwesomeIcon className="text-2xl " icon={faExclamationCircle} />} enableMultiContainer={true} position="bottom-left" autoClose={5000} hideProgressBar={false} closeOnClick rtl={false} pauseOnFocusLoss={false} draggable pauseOnHover={false} />
                ...
                {emailVarification ? <EmailVarification email={email} /> : ""}
            </div>
        </Grow>
    )
}
