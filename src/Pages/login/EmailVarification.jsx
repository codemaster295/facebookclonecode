...
const EmailVarification = ({ email }) => {
    ...
    const handleResendOtp = () => {
        ...
        axios
            .post("localhost:8080/api/v1/signup/resendOtp", email)
            .then((d) => {
                console.log(d.data)
                if (d.data) {
    ...
    return (
        <div className="fixed top-0 left-0 bg-white w-full h-full">
            <div className="shadow-2xl flex flex-col md:w-6/12 w-full lg:w-1/3 absolute space-y-5 top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white border-none rounded-lg p-5 ">
                ...
                <span className="text-btn-blue cursor-pointer text-xs font-semibold" onClick={handleResendOtp}>Click here resend the otp</span>
